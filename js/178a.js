...
if (g_debug == 1) {
	...
	data_file_name = 'a.uf8'/*tpa=http://cfd.178.com/s/moniqi/renlei/a.uf8*/;
}
...
var strKeiretuStr = new Array('n/a', '斬打', '射撃');
...
var disp_popup = true;
var loaded = false;
var main_skill_num = 48;
var all_skill_num = 68;
var max_upg_num = 3;
...
function makeToolTipWnd()
{
	...
	str += '<img src="s0_e.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/s0_e.gif*/ height="64" width="64" id="dw_img">\n';
	str += '</div>\n';
	str += '<div id="dw_kind">\n';
	str += '種類\n';
	str += '</div>\n';
...
function getWakeupName(i, wup)
{
	...
	if (i == 1) {
		if (ch1 == 1) return 'アタックマシン';
		if (ch1 == 2) return 'シューティングマシン';
	}
	if (i == 2) {
		if (ch1 == 1) {
			if (ch2 == 1) return 'ツーハンドマシーン';
			if (ch2 == 2) return 'ワンハンドマシーン';
		}
		if (ch1 == 2) {
			if (ch2 == 1) return 'スピードマシン';
			if (ch2 == 2) return 'マインマシン';
		}
	}
	if (i == 3) {
		if (ch1 == 1) {
			if (ch2 == 1) if (ch3 == 1) return 'バトルマーシン';
			if (ch2 == 2) if (ch3 == 1) return 'シールドマシーン';
		}
		if (ch1 == 2) {
			if (ch2 == 1) if (ch3 == 1) return 'スナイプマシーン';
			if (ch2 == 2) if (ch3 == 1) return 'キャノンマシーン';
		}
	}
	return '未覚醒';
}
...
function buildSkillBox(idx)
{
	...
	str += '<div id="sp';
	str += idx;
	str += '" class="plus"><img src="plus.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/plus.gif*/></div>\n';
	str += '</div>\n';
	str += '</div>\n';
	return str;
}
...
function makePage1Frame()
{
	...
	str += buildLongLeftArrow(13, 1);
	str += '</td><td>\n';
	str += buildSkillBox(12);
	str += buildDownArrow(14, 1);
	str += '</td><td>\n';
	str += buildRightArrow(18, 1);
	str += '</td><td>\n';
	str += buildSkillBox(22);
	str += buildDownArrow(23, 1);
	str += '</td><td>\n';
	str += buildRightArrow(26, 1);
	str += '</td></tr><tr><td>\n';
...
{
	var str = '<table><tr><td rowspan="2">\n';
	str += buildLongLeftArrow(31, 1);
	str += '</td><td>\n';
	str += buildSkillBox(30);
	str += buildDownArrow(32, 1);
	str += '</td><td>\n';
	str += buildRightArrow(36, 1);
	str += '</td><td>\n';
	str += buildLeftArrow(41, 1);
	str += '</td><td>\n';
	str += buildSkillBox(40);
	str += buildDownArrow(43, 1);
	str += '</td><td>\n';
	str += buildRightArrow(46, 1);
	str += '</td></tr><tr><td>\n';
	...
	for (i = 1; i <= 6; ++i) {
		...
		str += '<div id="won' + i + '">\n';
			...
			str += ' <div class="wcell_on2">\n';
				str += '  <div class="icon"><img src="w0_e.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/w0_e.gif*/ id="wa' + i + '"></div>\n';
				str += '  <div id="wan' + i + '" class="name">name1</div>\n';
				str += '  <div id="wal' + i + '" class="lv">[0/10]</div>\n';
			str += ' </div>\n';
			str += ' <div class="wcell_on3">\n';
				str += '  <div class="icon"><img src="w0_d.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/w0_d.gif*/ id="wb' + i + '"></div>\n';
	...
	if (ar != null) {
		...
		if (skill_list[idx].needs > 0) {
			if (isAbleOnArrow(idx)) {
				src = src.replace('404.html'/*tpa=http://cfd.178.com/s/moniqi/renlei/_d.gif*/, '404.html'/*tpa=http://cfd.178.com/s/moniqi/renlei/_e.gif*/);
			} else {
				src = src.replace('404.html'/*tpa=http://cfd.178.com/s/moniqi/renlei/_e.gif*/, '404.html'/*tpa=http://cfd.178.com/s/moniqi/renlei/_d.gif*/);
			}
			ar.src = src;
		}
	}
...
function getWakeupIconFileName(wup, offset1, offset2, enable)
{
	...
	if (idx <= 0) {
		return 'w0_d.gif'/*tpa=http://cfd.178.com/s/moniqi/renlei/img/w0_d.gif*/;
	}
...
//	var tab3 = document.getElementById('tab3');
	...
	if (id == 'tab1') {
		...
		hidePage(4);
		showPage(1);;
		tab1.style.backgroundImage = 'url("t1_e.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/t1_e.gif*/)';
		tab2.style.backgroundImage = 'url("t2_d.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/t2_d.gif*/)';
		tab4.style.backgroundImage = 'url("t4_d.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/t4_d.gif*/)';
	} else if (id == 'tab2') {
		if (now_select_page == 2) {
			return;
		}
		...
		hidePage(4);
		showPage(2);;
		tab1.style.backgroundImage = 'url("t1_d.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/t1_d.gif*/)';
		tab2.style.backgroundImage = 'url("t2_e.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/t2_e.gif*/)';
		tab4.style.backgroundImage = 'url("t4_d.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/t4_d.gif*/)';
	} else if (id == 'tab4') {
		...
		tab1.style.backgroundImage = 'url("t1_d.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/t1_d.gif*/)';
		tab2.style.backgroundImage = 'url("t2_d.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/t2_d.gif*/)';
		tab4.style.backgroundImage = 'url("t4_e.gif"/*tpa=http://cfd.178.com/s/moniqi/renlei/img/t4_e.gif*/)';
		updateAllWakeupSkillBoxData();
	}
}
